      ...
      <div className="pl-10 xl:pl-0 relative">
        ...
        <div className="flex flex-col gap-5 xl:w-[76rem]">
          {games.map((game, index) => (
            <div
              className={`bg-muted p-3 flex flex-col items-center gap-2 rounded relative max-w-xl group even:self-end ${index !== 0 ? "xl:-mt-28" : ""}`}
              key={index}
            >
              <div className="gap-2 absolute xl:group-odd:-right-[calc(2rem+.1rem)] xl:-left-[calc(2rem+.1rem)] xl:group-odd:left-auto xl:translate-x-0 top-5 h-16 w-16 left-0 -translate-x-1/2 -z-10 border-foreground border-b-[.25rem] border-x-[.2rem] rounded-b-[20px]"></div>
              {game}
            </div>
          ))}
        </div>
